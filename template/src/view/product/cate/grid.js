define(function(require){
    ...
    o.init = function(domid){
        ...
        store = new mwt.Store({
            proxy: new mwt.HttpProxy({
                ...
                url : ajax.getAjaxUrl("t_ruixi_product_cate&action=query")
            })
        });
        grid = new MWT.Grid({
            render      : gridid,
            ...
            cm: new MWT.Grid.ColumnModel([
                {head:'图片',dataIndex:'imgurl',width:100,align:'center',render:function(v,item){
                    return '<img src="'+v+'" style="max-width:70px;max-height:70px;border:solid 1px #ccc;">';
                }},

                {head:'分类ID',dataIndex:'id',width:60,align:'left',render:function(v,item){
                    return '#'+v;
                }},
                {head:'分类名称',dataIndex:'name',width:100,align:'left',render:function(v,item){
                    var url = dz.siteurl+'plugin.php?id=ruixi:product&mod=list&cate='+item.id;
                ...
                {head:'操作',dataIndex:'id',align:'right',sort:false,width:100,render:function(v,item){
        ...
        store.on('load',function(){
            ...
            jQuery('[name=adenable]').unbind('change').change(function(){
                ...
                ajax.post('t_ruixi_product_cate&action=setEnable',{id:id,enabled:enable},function(res){
    ...
    {/*{{{*/
        ...
        mwt.confirm('确定要删除吗?',function(res){
            if (!res) return;
            ajax.post('t_ruixi_product_cate&action=remove',{id:id},function(res){
